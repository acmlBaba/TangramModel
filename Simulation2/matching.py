...
out= np.load("./prediction/top5_"+str(sys.argv[1])+'_'+str(sys.argv[2])+".npy", allow_pickle=True)
ou=[]
...
base = "./out_img2img/img2imgResult/img2img_pic_"
...
for i in range(0,6):
    for j in range(0,8):
        if ou[i][j][2]==ou[i][j][4]:
                ...
                image_s.append(base+ str(ou[i][j][2]) + "_" + str(ou[i][j][3]) + "_"+str(sys.argv[1])+'_'+str(sys.argv[2])+".png")
